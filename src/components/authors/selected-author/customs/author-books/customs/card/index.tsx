...
// export default Card;
const Card = ({ book, authorName }) => {
  return (
    <div className="w-[184px] h-[342px]">
      <div className="relative">
        <img
          src={book.imgUrl || "https://via.placeholder.com/184x255"}
          alt={book.name}
          className="h-[255px] rounded-[8px] object-cover"
        />
        <div className="absolute bg-[#EF7F1A] w-10 h-10 rounded-full flex items-center justify-center bottom-2 right-5 cursor-pointer">
          <img src="/icons/bag.svg" alt="bag" />
        </div>
      ...
      <div className="mt-[10px]">
        ...
        <p className="text-[#828282] text-sm">{authorName}</p>
        <h3 className="text-[#EF7F1A] text-base font-bold">
          {book.bookPrice} so'm
        </h3>
      </div>
    </div>
